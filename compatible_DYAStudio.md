...
## 2. `cool642tb_R.conf`（Central側）追記
...
## 3. `cool642tb_L.conf`（Peripheral側）追記
...
- `zmk-behavior-runtime-sensor-rotate` は `cw-binding` / `ccw-binding` を使う。
  - `bindings = <...>` ではビルドエラーになる。
- `badjeff/zmk-pmw3610-driver` では、設定によっては `evt-type`, `x-input-code`, `y-input-code` の定義が必要。
- 古いZMK世代（例: workflow 側が `v0.2.1`）と混在させるとリンクエラーになりやすい。
